...
namespace PowerwallSniffer.DatabaseProvider.TimescaleDB.Repository
{
    ...
    public class SolarRepository  : IRepository<SolarModel>
    {
        private readonly string _connectionString;

        public SolarRepository(string connectionString)
        {
        ...
        public async Task Insert(SolarModel item)
        {
            await using var dbConnection = Connection;
            await dbConnection.OpenAsync();

            await using (var command = new NpgsqlCommand(@"
                            INSERT INTO home_data.solar
                                (
